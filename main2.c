...
#define nightroom_music "music/1.mp3"
#define treasure_music "music/2.mp3"
...
char settings_items[3][30] = {"Difficulty Level", "Hero Color","Music"};
int menu_items_count = 6, selected_menu = 0;
int settings_count = 3;
...
int first_save = 1, last_score = 0, last_gold = 0;
Mix_Music *music = NULL;
int music_playing = 0;
char main_music[20] = "none";

// save nadashte bashe
// win test beshe
// load game dobare ba new game check beshe
// health bar asas time nabashe

// tedad enemy othagh ganj be diff
// show message posht ham
...
{
    int selected = 0;
    char items[5][30] = {"puzzle game", "palabras perdidas", "pixel fight", "stranger things", "Stop Music"};
    int x, y;
    getmaxyx(stdscr, y, x);
    y /= 2;
    clear();
    ...
    for (int i = 0; i < 5; i++)
    {
        if (i == selected)
            attron(COLOR_PAIR(2));
        mvprintw(y + (i - 2) * interval, (x - strlen(items[i])) / 2, "%s", items[i]);
        if (i == selected)
    ...
    while (1)
    {
        ...
        else if (c == '\n')
        {
            end_music();
            if (selected == 0)
                strcpy(main_music, "music/3.mp3");
            else if (selected == 1)
                strcpy(main_music, "music/4.mp3");
            else if (selected == 2)
                strcpy(main_music, "music/5.mp3");
            else if (selected == 3)
                strcpy(main_music, "music/6.mp3");
            else if (selected == 4)
...
void print_box(int y1,int y2,int x1,int x2)
{
    for(int i=y1;i<=y2;i++)
        for(int j=x1;j<=x2;j++)
        {
...
void game_info(game_struct *game, hero_struct *hero)
{
    ...
    int x, y;
    getmaxyx(stdscr, y, x);
    move(starty_map_draw + map_height + 2, 0);
    clrtoeol();
...
void save_game(game_struct *game, hero_struct *hero)
{
    ...
    while (fgets(line_data, 500, file2))
    {
        ...
        char username_line[100], password_line[100], email_line[100], date[50];
        sscanf(line_data, "%s %s %s %d %d %d %s", username_line, password_line, email_line, &score, &gold, &number_of_games, date);
        if (strcmp(username_line, user_username) == 0)
        {
            if (first_save)
            {
                ...
                number_of_games++;
                last_gold = hero->gold;
                last_score = hero->gold * 10;
                first_save = 0;
            }
            else
            {
                score += hero->gold * 10 - last_score;
                gold += hero->gold - last_gold;
                last_gold = hero->gold;
                last_score = hero->gold * 10;
            }
            sprintf(line_data, "%s %s %s %d %d %d %s %d\n", username_line, password_line, email_line, score, gold, number_of_games, date);
        }
...
int move_hero(game_struct *game, hero_struct *hero, int c)
{
    ...
    if (next != nothing && next != pillar && next != wall[0] && next != wall[1] && (game->map[floor_number][hero->y][hero->x] != corridor || next != stairs) && x >= 0 && x < map_width && y >= 0 && y < map_height)
    {
        ...
        else
        {
            ...
            {
                enter_room(game, x, y);
                if (strcmp(game->room[floor_number][this_room_number[floor_number]].theme, nightmare) == 0 && now == corridor)
                    play_music(nightroom_music);
                else if (strcmp(game->room[floor_number][this_room_number[floor_number]].theme, treasure) == 0)
                    play_music(treasure_music);
            }
            if (next == corridor)
            {
                if ((now == door || now == opened_pass_door || now == hidden_door || now == found_door) && (strcmp(game->room[floor_number][this_room_number[floor_number]].theme, nightmare) == 0 || strcmp(game->room[floor_number][this_room_number[floor_number]].theme, treasure) == 0))
                {
                    end_music();
                    play_music(main_music);
                }
...
void print_leaderboard_page(int start, int end, user users[], int order[])
{
    start--;
    ...
    int vert = 3;
    getmaxyx(stdscr, y, x);
    int starty = (y - (end - start + 1) * (vert + 1)) / 2;
    int startx = x / 2;
    mvprintw(starty - 3, (x - strlen("Leaderboard")) / 2, "Leaderboard");

    x /= 2;
    int interval = 20;
    int usercol = x - 4 * interval, scorecol = x - 2 * interval, goldcol = x, gamecol = x + 2 * interval, datecol = x + 4 * interval;
    mvprintw(starty - 1, usercol, "Username");
    mvprintw(starty - 1, scorecol, "Score");
    mvprintw(starty - 1, goldcol, "Gold");
    mvprintw(starty - 1, gamecol, "Games");
    mvprintw(starty - 1, datecol, "Date");
    starty += 2;
    for (int i = start; i < end; i++)
    {
        if (i == 0)
        {
            ...
            mvprintw(starty + (i - start) * (vert + 1), usercol - strlen("Champion") - 5, "%s %lc", "Champion", utreasure_char);
        }
        else if (i == 1)
        {
            attron(A_BOLD);
            attron(COLOR_PAIR(blue_color));
            mvprintw(starty + (i - start) * (vert + 1), usercol - strlen("Runner-up") - 5, "%s %lc", "Runner-up", L'🥈');
        }
        else if (i == 2)
        {
            attron(A_BOLD);
            attron(COLOR_PAIR(red_color));
            mvprintw(starty + (i - start) * (vert + 1), usercol - strlen("Third_place") - 5, "%s %lc", "Third_place", L'🥉');
        }
        ...
        mvprintw(starty + (i - start) * (vert + 1), usercol, "%d.  %s", i + 1, users[order[i]].username);
        mvprintw(starty + (i - start) * (vert + 1), scorecol, "%d", users[order[i]].score);
        mvprintw(starty + (i - start) * (vert + 1), goldcol, "%d", users[order[i]].gold);
        mvprintw(starty + (i - start) * (vert + 1), gamecol, "%d", users[order[i]].number_of_games);
        mvprintw(starty + (i - start) * (vert + 1), datecol, "%s", users[order[i]].date);
        attroff(A_BOLD);
...
void leaderboard()
{
    ...
    while (fgets(line, sizeof(line), file))
    {
        if (line[0] == '\n')
            break;
        sscanf(line, "%s %s %s %d %d %d %s", users[number].username, users[number].password, users[number].email, &users[number].score, &users[number].gold, &users[number].number_of_games, users[number].date);
        order[number] = number;
        number++;
    }
    fclose(file);
    ...
    int page_number = 0;
    print_leaderboard_page(1, 10, users, order);
    ...
    while (1)
    {
        ...
        if (c == KEY_RIGHT)
        {
            if ((page_number + 1) * 10 < number)
            {
                page_number++;
                print_leaderboard_page(page_number * 10 + 1, min(number, (page_number * 10 + 10)), users, order);
            }
        }
        if (c == KEY_LEFT)
        {
        ...
        if (ch != -1)
        {
            ...
            else if (ch == '\n')
            {
                ...
                else
                {
                    ...
                    fprintf(file, "%s %s %s 0 0 0 %s\n", username, password, email, date_str);
                    fclose(file);
                    sleep(1);
                    game_menu();
                    game_menu_input();
    ...
    while (1)
    {
        ...
        if (x != ERR)
        {
            ...
            else if (x == '\n' || x == KEY_RIGHT)
            {
                ...
                else if (strcmp(settings_items[selected_settings], "Music") == 0)
                {
                    music_menu();
                    settings_menu();
                }
...
void profile()
{
    ...
    int x, y;
    getmaxyx(stdscr, y, x);
    char print_list[7][50];
    strcpy(print_list[0], "Profile");
    sprintf(print_list[1], "Username: %s", username_line);
    sprintf(print_list[2], "Email: %s", email_line);
    sprintf(print_list[3], "Score: %d", score);
    sprintf(print_list[4], "Gold: %d", gold);
    sprintf(print_list[5], "Number of games: %d", number_of_games);
    sprintf(print_list[6], "Date: %s", date);
    int interval_y=5;
    print_box(y/2-5*interval_y,y/2+5*interval_y,x/2-25,x/2+25);
    for (int i = 0; i < 7; i++)
    {
        if(i==0)
            attron(A_BOLD);
        mvprintw(y / 2 + interval_y * (i - 3), (x - strlen(print_list[i])) / 2, "%s", print_list[i]);
        if(i==0)
            attroff(A_BOLD);
    }
    refresh();
